...
parser.add_argument("--model_type", type=str, default=None, help="")
...
def _checkpoint_mismatches_model_type(checkpoint_path, model_type):
    if not checkpoint_path or not model_type:
        return False
    path = str(checkpoint_path).lower()
    model_type = str(model_type).lower()
    if model_type == "transformer" and "/mamba/" in path:
        return True
    if model_type == "mamba" and "/transformer/" in path:
        return True
    return False
...
if args_cli.model_type:
    ...
    if _checkpoint_mismatches_model_type(eval_cfg.get("model_checkpoint"), args_cli.model_type):
        eval_cfg.model_checkpoint = None
_prepare_eval_cfg(eval_cfg)
if eval_cfg.get("model_checkpoint"):
    print(f"[INFO] Eval checkpoint: {eval_cfg.model_checkpoint}")
else:
    print("[WARN] Eval checkpoint not set.")
...
def _update_episode_state(env, dones, success_threshold, target_pos):
            ...
            while simulation_app.is_running() and rollouts_done < eval_cfg.num_rollouts:
                ...
                if torch.any(done_mask):
                    ...
                    if (
                        ...
                        # Video file is saved directly by the RecordVideo wrapper.
                        pass
